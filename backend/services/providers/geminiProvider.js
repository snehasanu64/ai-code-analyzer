/**
 * Real Gemini provider. Requires a per-request apiKey (the user's own key,
 ...
 */
const { buildPrompt, callGemini } = require("./llmHelper");

const makeMethod = (name) => async (args) => {
  if (!args.apiKey) throw new Error("No Gemini API key configured for this user.");
  const { system, user } = buildPrompt(name, args);
  return callGemini({ apiKey: args.apiKey, model: args.model, system, user });
};
